...
//			if(section == -1) {
//				var point = points.eq(0);
//				barWidth = point.offset().left + (point.width() / 12);
//			}
//			// if after end
...
//				//barWidth = startPointX + (startPointWidth / 2.48) + (startPointWidth * sectionProgress);
//			}
...
//		var targetY = $('h2').eq(sectionIndex).offset().top - (triggerPoint * .48);
//		$('html, body').animate({scrollTop:targetY}, 700, 'easeInOutCubic');
//	});
//
//});
...
$(document).ready(function(){
	...
	function setPosition(){
		if(container.is(':visible')) {
			...
			// if within document
				...
				var scrollY = currentPosition - startSectionY;
				var sectionProgress = scrollY / sectionLength;
				barWidth = startPointX + (startPointWidth / 2.2) + (startPointWidth * sectionProgress);
			}
			barWidth -= indicator.offset().left;
			indicator.css('width', barWidth+'px');
		}
	...
	points.click(function(){
		var sectionIndex = points.index($(this));
		var targetY = $('h2').eq(sectionIndex).offset().top - (triggerPoint * .72);
